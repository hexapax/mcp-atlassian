...
class TestCallToolMcpErrorMessages:
    ...
    @pytest.mark.anyio
    async def test_delete_tool_blocked(self, mcp_server: AtlassianMCP) -> None:
        """Delete tool filtered shows ALLOW_DELETE_TOOLS message."""
        delete_tool = self._make_mock_tool(
            "jira_delete_issue",
            {"jira", "delete", "toolset:jira_issues"},
        )
        with (
            patch.object(
                mcp_server,
            ...
            patch.object(
                ...
                "get_tools",
                new_callable=AsyncMock,
                return_value={"jira_delete_issue": delete_tool},
            ),
            pytest.raises(NotFoundError, match="ALLOW_DELETE_TOOLS"),
        ):
            await mcp_server._call_tool_mcp("jira_delete_issue", {})
